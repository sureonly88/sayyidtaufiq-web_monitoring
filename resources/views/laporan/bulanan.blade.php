			...
			<tr>
				...
				<td align="right" >{{ number_format($r->jumlah) }}</td>
					<?php $setor=0;
						...
						//
						//if($r->tipe=='4'){$share=250;}
						//else if($r->tipe=='1'&&$r->jenis_transaksi!='PDAM_BANDARMASIH'){
						//	$share=$byadmin-200;
						//}else if($r->tipe=='1'&&$r->jenis_transaksi=='PDAM_BANDARMASIH'){
						//	$share=$byadmin;
						//}
						//else{$share=$byadmin*50/100;}

						if($r->jenis_transaksi=='PDAM_BANDARMASIH'){
							$share=$r->pdam;
						}else if($r->jenis_transaksi=='PLN_POSTPAID'){
							$share=$r->pln_postpaid;
						}else if($r->jenis_transaksi=='PLN_PREPAID'){
							$share=$r->pln_prepaid;
						}else if($r->jenis_transaksi=='PLN_NONTAGLIS'){
							$share=$r->pln_nontaglis;
						}
						//

						//if($r->tipe=='4'){$share=250;}
						//else if($r->tipe=='1'){$share=$r->byadmin;}
						//else{$share=$r->byadmin*50/100;}
