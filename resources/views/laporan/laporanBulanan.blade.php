@extends('menu')
@section('header')
    <link href="{{URL::asset('adminlte/plugins/datatables/dataTables.bootstrap.css')}}" rel="stylesheet" type="text/css" />
    <link rel="stylesheet" href="{{URL::asset('adminlte/plugins/select2/select2.min.css')}}">
    <link href="{{URL::asset('adminlte/plugins/datepicker/datepicker3.css')}}" rel="stylesheet" type="text/css" />
@endsection

@section('body')
<h3 class="box-title">Laporan Bulanan</h3>
<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data"> 
<div class="box box-solid box-primary">
  <div class="box-header">
    <h3 class="btn btn disabled box-title">
    <i class="fa fa-search"></i> Filter </h3>
  	<a class="btn btn-default btn-sm pull-right" data-widget='collapse' data-toggle="tooltip" title="Collapse" style="margin-right: 5px;">
  	<i class="fa fa-minus"></i></a>
  </div>	
	<div class="box-body">
    	<div class="form-group">
      	@if (session('auth')->level!=2)
			<div class="col-sm-6">	
				<label class="control-label" for="input-tipe">Jenis</label>
				<select name="tipe" id="tipe" class="form-control select2">
					<option>ALL</option>
					<option value="1">Loket Pedami</option>
					<option value="2">Loket Luar</option>
					<option value="3">Loket Petugas Lapangan Pedami</option>
					<option value="4">Switching</option>
					<option value="5">Loket Luar (H+1)</option>
					<option value="6">Switching Deposit</option>
				</select>				
			</div>
			<div class="col-sm-6">	
				<label class="control-label" for="input-status">Loket</label>
				<input  name="loket" id="loket" type="text" class="form-control">				
			</div>
			<div class="col-sm-6">	
				<label class="control-label" for="input-tipe">Jenis Transaksi</label>
				<select name="jenis" id="jenis" class="form-control select2">
					<option>ALL</option>
					<option value="PDAM_BANDARMASIH">PDAM</option>
					<option value="PLN_POSTPAID">PLN POSTPAID</option>
					<option value="PLN_PREPAID">PLN PREPAID</option>
					<option value="PLN_NONTAGLIS">PLN NONTAGLIS</option>
					<option value="PLN">PLN ALL</option>
				</select>				
			</div>
		@endif	
			<div class="col-sm-6">	
				<label class="control-label" for="input-status">Bulan</label>
				<input  id="tgl"  name="tgl" type="text" class="form-control datepicker" >				
			</div>
    	</div>
    	<div class="form-group">
    		<div class="col-sm-5">
		        <button type="button" name="filter" id="filter"  class="btn btn-primary"><i class="glyphicon glyphicon-search"></i> Filter</button>	
		    </div>
		</div>    
  </div>
</div>
</form>

<div class="box box-solid box-primary">
	<div class="box-header">
		<h3 class="btn disabled box-title">
		<i class="fa fa-laptop"></i> Rekap Transaksi</h3>
	</div>		
<div class="box-body">
<table id="example2" class="table table-bordered table-striped">
<thead>
	<tr>
		<th class="text-center" style="min-width:30px">No</th>
		<th class="text-center" style="min-width:100px">Tahun</th>
		<th class="text-center" style="min-width:100px">Bulan</th>
		<th class="text-center" style="min-width:100px">Nama Loket</th>
		<th class="text-center" style="min-width:100px">Jenis Transaksi</th>
		<th class="text-center" style="min-width:50px">Jumlah</th>
		<th class="text-center" style="min-width:100px">Tagihan</th>
		<th class="text-center" style="min-width:100px">Admin</th>
		<th class="text-center" style="min-width:100px">Jumlah Admin</th>
		<th class="text-center" style="min-width:100px">Share</th>
		<th class="text-center" style="min-width:100px">Jumlah Share</th>
		<th class="text-center" style="min-width:100px">Detail</th> 	
	</tr>
</thead>
<tbody>
	<tr>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
	</tr>
</tbody>
<tfoot>
	<tr>
		<th class="text-right" colspan="5">Total</th>
		<th></th>
		<th></th>
		<th></th>
		<th></th>
		<th></th>
		<th></th>
		<th></th>
	</tr>
</tfoot>
</table>
<button class="btn btn-primary" type="button" onclick="cetakPdfBulanan()">
	<span>Cetak</span>
</button>

</div>
</div>


@endsection
@section('plugins')
<script src="{{URL::asset('adminlte/plugins/datatables/jquery.dataTables.min.js')}}" type="text/javascript"></script>
<script src="{{URL::asset('adminlte/plugins/datatables/dataTables.bootstrap.min.js')}}" type="text/javascript"></script>
<script src="{{URL::asset('adminlte/plugins/select2/select2.full.min.js')}}"></script>	
<script src="{{URL::asset('adminlte/plugins/datepicker/bootstrap-datepicker.js')}}" type="text/javascript"></script>

@endsection

@section('footer')
	<script>	
	$("#example2").dataTable({
	   	"scrollY": 1000,
	   	"scrollX": true,
	   	"scrollCollapse": true,
	   	"autoWidth": true,
	   	"ordering" : false,
	   	"info" : false
	   });

	$('.select2').select2();
	$(document).ready(function(){
	    $(".datepicker").datepicker( {
		    format: "yyyy-mm",
		    startView: "months", 
		    minViewMode: "months",  
	    	autoclose: true
		});
	 });	
	

	var data = <?= json_encode($lokets) ?>;
	var loket = $('#loket').select2({
		data: data,
		placeholder: 'Pilih Loket',
		allowClear: true,
		minimumInputLength: 0,
		formatSelection: function(data) { return data.text },
		formatResult: function(data) {
			return  '<span class="label label-info">'+data.id+'</span>'+
						'<strong style="margin-left:5px">'+data.text+'</strong>';

		}
	});

	$('#tipe').change(function(){
		//loket.select2("destroy");
		$.getJSON("{{ url('ajaxLoketsTipe')}}/"+$(this).val(), function(data) {
			loket.select2({
				data: data,
				placeholder: 'Pilih Loket',
				allowClear: true,
				minimumInputLength: 0,
				formatSelection: function(data) { return data.text },
				formatResult: function(data) {
					return  '<span class="label label-info">'+data.id+'</span>'+
								'<strong style="margin-left:5px">'+data.text+'</strong>';
				}
			});
		});
	});
	//format desimal
	function formatDesimal(num){
	    var n = num.toString(), p = n.indexOf('.');
	    return n.replace(/\d(?=(?:\d{3})+(?:\.|$))/g, function($0, i){
	        return p<0 || i<p ? ($0+',') : $0;
	    });
	}
	//filter
    $("#filter").on('click',function(){
    if ($("#tgl").val()==''){
		alert("Pilih Bulan");
	}else{	
      	var tahun = $("#tgl").val().slice(0, 4);
		var bulans = $("#tgl").val().slice(5, 8);
		var bulan = parseInt(bulans);
		var loket = $("#loket").val();
		if(!loket){
			loket = "-";
		}
		var jenis = $("#jenis").val();
		var tipe = $("#tipe").val();
		var nomor=0;

		$('#example2').dataTable( {
	    "ajax": "{{ url('/laporan/ajaxlapBulanan') }}/"+tahun+"/"+bulan+"/"+loket+"/"+jenis+"/"+tipe,
	    "destroy": true,
	    "columns": [
	        { "data": "aksi" },
	        { "data": "aksi" },
	        { "data": "aksi" },
	        { "data": "loket_name" },
	        { "data": "jenis_transaksi" },
	        { "data": "jumlah",render: $.fn.dataTable.render.number(',', '.', 0, '') },
	        { "data": "tagihan",render: $.fn.dataTable.render.number(',', '.', 0, '') },
	        { "data": "byadmin" },
	        { "data": "admin",render: $.fn.dataTable.render.number(',', '.', 0, '') },
	        { "data": "aksi" },
	        { "data": "aksi" },
	        { "data": "aksi" }
	    	],
		    "aoColumnDefs": [ 
		     {
		      "aTargets": [ 0 ],
		      "mRender": function (data, type, full) {
		          nomor=nomor+1;
		          return nomor;
			    }
		      },
		      {
		      "aTargets": [ 1 ],
		      "mRender": function (data, type, full) {
		          return tahun;
			    }
		      },
		      {
		      "aTargets": [ 2 ],
		      "mRender": function (data, type, full) {
		          return bulan;
			    }
		      },
		      {
		      "aTargets": [ 7 ],
		      "mRender": function (data, type, full) {
		          if (full.jenis_transaksi!='PDAM_BANDARMASIH'){
		            var byadmin=2500;
		          }else{
		            var byadmin =full.byadmin;
		          }
		          return formatDesimal(byadmin);
			    }
		      },
		      {
		      "aTargets": [ 9 ],
		      "mRender": function (data, type, full) {
		          	if(full.jenis_transaksi=='PDAM_BANDARMASIH'){
						var share=full.pdam;
					}else if(full.jenis_transaksi=='PLN_POSTPAID'){
						var share=full.pln_postpaid;
					}else if(full.jenis_transaksi=='PLN_PREPAID'){
						var share=full.pln_prepaid;
					}else if(full.jenis_transaksi=='PLN_NONTAGLIS'){
						var share=full.pln_nontaglis;
					}
					else if(full.jenis_transaksi=='PLN_POSTPAID_N'){
						var share=full.pln_postpaid_n;
					}else if(full.jenis_transaksi=='PLN_PREPAID_N'){
						var share=full.pln_prepaid_n;
					}else if(full.jenis_transaksi=='PLN_NONTAGLIS_N'){
						var share=full.pln_nontaglis_n;
					}
					if(!share){share=0}

		          return formatDesimal(share);
			    }
		      }, 
		      {
		      "aTargets": [ 10 ],
		      "mRender": function (data, type, full) {
		          	if(full.jenis_transaksi=='PDAM_BANDARMASIH'){
						var share=full.pdam;
					}else if(full.jenis_transaksi=='PLN_POSTPAID'){
						var share=full.pln_postpaid;
					}else if(full.jenis_transaksi=='PLN_PREPAID'){
						var share=full.pln_prepaid;
					}else if(full.jenis_transaksi=='PLN_NONTAGLIS'){
						var share=full.pln_nontaglis;
					}
					else if(full.jenis_transaksi=='PLN_POSTPAID_N'){
						var share=full.pln_postpaid_n;
					}else if(full.jenis_transaksi=='PLN_PREPAID_N'){
						var share=full.pln_prepaid_n;
					}else if(full.jenis_transaksi=='PLN_NONTAGLIS_N'){
						var share=full.pln_nontaglis_n;
					}
					if(!share){share=0}
					var jumlah_share=parseFloat(share)*parseFloat(full.jumlah);

		          return formatDesimal(jumlah_share);
			    }
		      },
		      {
		      "aTargets": [ 11 ],
		      "mRender": function (data, type, full, level) {
		          var formmatedvalue=" <a href='{{ url('laporan/bulanan/pdfDetail')}}/"+tahun+"/"+bulan+"/"+full.loket_code+"/"+full.jenis_transaksi+"'" ;
			      formmatedvalue=formmatedvalue+" target='_blank' class='btn btn-warning btn-xs'>PDF</a>";
			      formmatedvalue=formmatedvalue+" <a href='{{ url('laporan/bulanan/excellDetail')}}/"+tahun+"/"+bulan+"/"+full.loket_code+"/"+full.jenis_transaksi+"'" ;
			      formmatedvalue=formmatedvalue+" target='_blank' class='btn btn-success btn-xs'>Excell</a>";
		          return formmatedvalue;
			    }
		      },
		    ],
		    "footerCallback": function ( row, data, start, end, display ) {
	            var api = this.api(), data;
	            // Remove the formatting to get integer data for summation
	            var intVal = function ( i ) {
	                return typeof i === 'string' ?
	                    i.replace(/[^\d.-]/g, '') * 1 :
	                    typeof i === 'number' ?
	                        i : 0;
	            };	 
	            // Total over all pages
	            jumlah = api.column( 5 ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );
	            tagihan = api.column( 6 ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );
	            admin = api.column( 8 ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );
	            jumlah_share = api.column( 10 ).cache('search').reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );

	            // Update footer

	            $( api.column( 5 ).footer() ).html('Rp '+formatDesimal(jumlah));
	            $( api.column( 6 ).footer() ).html('Rp '+formatDesimal(tagihan));
	            $( api.column( 8 ).footer() ).html('Rp '+formatDesimal(admin));
	            $( api.column( 10 ).footer() ).html('Rp '+formatDesimal(jumlah_share));
        	},
	        "scrollY": 1000,
		   	"scrollX": true,
		   	"scrollCollapse": true,
		   	"autoWidth": true,
		   	"ordering" : false,
		   	"info" : false,
	   		"lengthChange" :true,		
		});	
	}  
	}); 

	function cetakPdfBulanan(){
		var tahun = $("#tgl").val().slice(0, 4);
		var bulans = $("#tgl").val().slice(5, 8);
		var bulan = parseInt(bulans);
		var loket = $("#loket").val();
		if(!loket){
			loket = "-";
		}
		var jenis = $("#jenis").val();
		var tipe = $("#tipe").val();
		//alert(bulan);
	   	window.open("{{ url('laporan/bulanan/pdf') }}/"+tahun+"/"+bulan+"/"+loket+"/"+jenis+"/"+tipe,'_blank');
	}
</script>
@endsection